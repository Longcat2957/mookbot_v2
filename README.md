...
각 앱은 Docker 이미지로 빌드 → Docker Hub (`longcat1132/mookbot-{api,bot,activity}`) → VPS docker compose.

### 버전 관리

루트 `package.json` 의 `version` 이 단일 SoT. 모든 Docker 이미지가 이 버전 + `latest` 두 태그로 빌드/푸시됨.

```bash
# 버전 bump (lockfile 갱신, git tag 자동 안 만듦)
pnpm version:patch       # 0.1.0 → 0.1.1
pnpm version:minor       # 0.1.0 → 0.2.0
pnpm version:major       # 0.1.0 → 1.0.0

# 빌드 + 푸시 (X.Y.Z + latest 두 태그)
pnpm docker:build        # 3 이미지 로컬 빌드
pnpm docker:push         # 3 이미지 푸시
pnpm docker:release      # 빌드 + 푸시 한방에

# 표준 릴리즈 흐름
pnpm version:patch && pnpm docker:release && git commit -am "release v$(node -p 'require(\"./package.json\").version')"
```

VPS 배포는 `latest` 를 따라가며, 특정 버전 고정이 필요하면 `~/deploy/docker-compose.yml` 의 `image:` 태그를 X.Y.Z 로 변경.

## 문서

- [`ROADMAP.md`](./ROADMAP.md) — 현재 진척 / 완료 / 백로그
