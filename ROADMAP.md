...
- **biome 잔존 에러 정리** — Activity/BalancePreview·PermsModal·AuctionBracket·ChampPickerModal·vite.config, api/me-riot-accounts·series 등 lint 정리 (행위 변경 0, `pnpm check` clean).

### Phase 38 — 경매내전 라이엇 아바타 전면 통일 + 솔로랭크 우선 + UI 통합 (v0.10.2)
- **모든 Auction 화면 UserAvatar 에 riot `profileIconUrl` 전달** — CAPTAIN_PICK / POINT_ALLOC / BIDDING / 유찰 / AuctionBracket / AuctionResult 전 구간 통일. Phase 36 의 시리즈 측 통일을 경매 측까지 확장.
- **매물 라이엇 fetch: 솔로랭크 우선 → 자유랭크 fallback** — 자유랭크는 듀오/팀 변수가 커 실력 평가 신뢰도 낮음. 솔로랭크 데이터가 있으면 그쪽을 표시.
- **CandidateInfo 카드 통합** — [현재 매물 hero] / [라이엇 연동] / [내전 기록] 3 섹션을 divider 로 한 카드 안에 합침 (기존 2-col 분리 → 단일 카드 세로 흐름).
- **BIDDING 입찰 입력 패널 흡수** — 별도 vertical stack 제거. 매물 진행 중인 팀 카드 상단 (ring-primary highlight) 안에 input + 낙찰/수동 버튼 inline. 운영자 시선 이동 최소화.
- **백엔드 응답 확장**: `AuctionTournamentDetail.teams[].captainProfileIconUrl` + `members[].profileIconUrl` + `unsold[].profileIconUrl`, `AuctionRecruitmentDetail.participants[].profileIconUrl` (`listMainRiotAccounts` batch fetch).

### Phase 37 — 경매 토너먼트 softDelete 시 종속 시리즈 cleanup (v0.10.1)
- **버그**: 토너먼트 cancel + soft-delete 후에도 COMPLETED 상태였던 종속 시리즈가 `deleted_at IS NULL` 로 살아남음. 같은 ID 의 RANKED 모집 → 시리즈 변환 시 `createSeries` 의 동일 ID 충돌 검사에 걸려 변환 실패.
- **fix**: `softDeleteAuctionTournament` 가 종속 series 도 같이 soft-delete. cancel 단계는 historical 보존 의도라 그대로 유지 (`softDelete` = "완전히 사라짐" 의미 유지). `/경매내전강제삭제` 봇 명령은 이미 `softDeleteAuctionTournament` 를 호출 → 자동으로 같이 fix.
...
- **이전 게임 픽 표시 (W3)** — Game 2/3 입력 시 챔프 셀에 `G1` `G2` 배지 + hover tooltip ("G1 1팀 MID 픽 (W)"). detail.games 에서 derived map.
- **Bo3 자동 사이드 반전 (W4)** — Game 2/3 진입 시 이전 게임 사이드 반대로 default. ⋯ dropdown 설정 토글 + localStorage 영속.
- **EntryEditing 자동 배치 + Undo/Redo (W5)** — `roles[]`/`history.topRole` 기반 client-side 자동 배치 + 셔플. 메모리 Undo stack (depth 20) + Ctrl+Z / Ctrl+Shift+Z.
- **KeyboardHints (W6)** — 공통 `kbd` 콜렉션 컴포넌트, dismissible (localStorage). PickBan / EntryEditing 화면에 통합.
- **PickBan split layout (D1)** — lg+ 에서 보드 좌 60% + 챔프 그리드 우 40% sticky. 한 화면 동시 가시 — 슬롯/그리드 스크롤 분리.
- **컴포넌트 톤 강화 (D2)** — SlotTile activeSlot ring-4 + pulse + glow / 사이드 결정 카드 BLUE-RED bg tint + kbd badge / TeamColumn BAN/PICK 영역 색 분리 (warning/success tint + 🚫/⚔️) / ResultPanel `steps-horizontal` 4단계 (사이드/밴/픽/승자) + 승자 카드 1/2 kbd + 기록 버튼 Ctrl+Enter kbd.
