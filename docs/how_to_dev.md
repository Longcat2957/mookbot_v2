...
### 2.3 Discord 안에서 로컬 코드로 Activity 띄우기 (cloudflared 터널)

Discord Activity 는 iframe HTTPS 강제 — 로컬 vite (`http://localhost:5173`)
를 그냥 못 씀. Discord 공식 권장 방식 = **cloudflared 터널 → URL Mapping**.
([discord.com/developers/docs/activities/development-guides/local-development](https://discord.com/developers/docs/activities/development-guides/local-development))
...
2. **별도 dev Discord application 발급** (운영 application 과 충돌 회피 — 같은 앱이면
   URL Mapping 변경이 운영자 모두에게 영향). Developer Portal → New Application.

3. **Discord 클라이언트 Developer Mode ON**
   - Web/Desktop: Settings → Advanced → Developer Mode ON
   - Mobile: User Profile → Appearance → Developer Mode ON
...
# 터미널 2 — 터널 (vite dev port)
cloudflared tunnel --url http://localhost:5173
# 출력 예시: https://funky-jogging-bunny.trycloudflare.com
```

4. **Discord Dev Portal → 본인 dev app → Activities → URL Mappings**:
   - Root `/` 의 TARGET 을 발급된 호스트로 (https:// 제외)
     예: `funky-jogging-bunny.trycloudflare.com`
   - Application URL Override 는 **OFF**
   - quick tunnel URL 은 매 실행 변경 → 매번 mapping 갱신
     (귀찮으면 named tunnel 등록)

5. **Discord 클라이언트** 보이스 채널 입장 → 로켓 아이콘 → Activity Shelf 에서
   본인 dev app 선택. iframe 안에서 vite dev 가 로드됨 (HMR 포함).

#### vite proxy 설정

`apps/activity/vite.config.ts` 의 `server.proxy` 가 `/api`, `/ws`, `/dd` 를
백엔드로 forward — production nginx 의 location 분배와 같은 효과. tunnel 은
:5173 만 열면 됨.

#### 한계

- quick tunnel 도메인은 매번 변경 → URL Mapping 갱신 자동화 어려움.
  named tunnel 이나 Cloudflare 계정 + custom domain 으로 안정화 가능.
- ngrok 도 동일하게 동작하지만 free-tier 도메인은 reclaim 가능 — cloudflared
  권장.
- iframe 안에서 DevTools: Web 클라이언트는 우클릭 inspect, Desktop 은
  PTB 클라이언트 + View → Developer → Toggle DevTools.
