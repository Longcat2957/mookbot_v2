...
-- ============================================================
-- DB safety notice (v0.14.1 — incident postmortem)
-- ============================================================
-- 이 파일 어디에도 DESTRUCTIVE statement (DROP TABLE, DELETE FROM (without WHERE),
-- ALTER TABLE ... DROP COLUMN, RENAME TO 등) 를 추가하지 마세요.
-- migrate.ts 가 prod D1 에 모든 statement 를 재실행하므로, destructive statement 가
-- schema.sql 에 있으면 매번 데이터 손실 위험. 신규 마이그레이션은 위의 "Idempotent
-- ALTER TABLE migrations" 섹션에 ALTER ADD COLUMN / CREATE INDEX 만 추가하세요.
--
-- 1회성 transition (예: 컬럼 rename, 테이블 split) 이 정말 필요하면:
--   (1) 별도 .sql 파일로 분리 (예: `migrations/2026-05-14-foo.sql`)
--   (2) 한 번 prod 에 직접 적용 후 그 파일 삭제 (재실행 불가)
--   (3) schema.sql 의 CREATE TABLE 정의만 새 schema 로 업데이트
