...
대시보드 진입 → "엔트리 대기" 카드 1 개 클릭
 → 빈 슬롯 보드 — 운영자가 직접 라인 밸런싱 (도메인 핵심 가치)
 → DnD 또는 Tap-to-Place 로 10 명 배정
 → 엔트리 제출 → 픽/밴 화면
```
**KPI**: 시간 ≤ 60 초. 자동 배치 제안 없음 — 수동 밸런싱이 운영자의
핵심 책임이며, history 기반 자동 매칭은 의도적 비목표(§10).
...
**자동 배치 제안 — 비목표 (Non-Goal)**:
운영자가 라인 밸런싱을 직접 하는 것이 도메인의 핵심 가치이므로
history 기반 자동 매칭 / 추천 배치는 도입하지 않는다. 운영자는
참가자의 전적 / 메인 라인을 카드에 표시된 정보로 보고 수동으로
판단해 배치한다 (§10 비목표 참조).
...
15. ~~자동 배치 제안~~ — **비목표**, 수동 밸런싱이 핵심 가치 (§10)
...
- [ ] **Flexibility & efficiency**: 단축키 + MY MAINS 우선 표시 + Tap-to-Place / DnD 동시 지원으로 파워 유저 가속 (단, 라인 밸런싱은 운영자 수동 책임)
...
- **엔트리 자동 배치 / 추천 매칭** — 운영자가 참가자의 전적 / 메인
  라인을 보고 라인 밸런싱을 직접 수행하는 것이 도메인의 핵심 가치.
  history.topRole 기반 자동 배치는 의도적으로 도입하지 않는다. UI 는
  카드에 충분한 정보 (전적, WR, 주 라인, 챔프) 를 제공해 운영자의
  수동 판단을 돕는 데 집중한다.
