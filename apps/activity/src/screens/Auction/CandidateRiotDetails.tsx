...
function RiotRankCard({ account }: { account: RiotAccount }) {
	...
	return (
		<div className="flex items-center gap-2 surface-quiet-soft rounded-md p-2">
			{emblemSrc && (
				<img
					src={emblemSrc}
					...
					width={64}
					height={64}
					className="size-11 shrink-0 object-contain drop-shadow"
					draggable={false}
					loading="lazy"
					decoding="async"
				/>
			...
			<div className="flex-1 min-w-0">
				...
				<div
					className={`text-base font-bold tabular-nums ${TIER_COLOR[ranked.tier] ?? "text-base-content"}`}
				>
					{ranked.tier} {ranked.rank}{" "}
					<span className="text-xs font-medium">{ranked.leaguePoints} LP</span>
				</div>
				...
				{account.masteries.map((m) => (
					...
					>
						<img
							...
							decoding="async"
						/>
						<div className="text-[11px] font-medium truncate w-full text-center">{m.name}</div>
