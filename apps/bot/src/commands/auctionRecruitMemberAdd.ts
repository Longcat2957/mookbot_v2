// /경매내전모집인원추가 — 운영자가 경매 모집에 멤버 강제 추가.
...
export const data = new SlashCommandBuilder()
	.setName("경매인원추가")
	.setDescription("경매 모집에 멤버를 한 명 추가합니다 (BalanceTeam 전용).")
...
export async function execute(interaction: ChatInputCommandInteraction): Promise<void> {
	...
	await addAuctionRecruitmentParticipant({ recruitmentId: id, userId: targetUser.id });
	await recordAudit({
		operatorId: interaction.user.id,
		action: "auction-recruitment.member-added",
		targetType: "auction-recruitment",
		targetId: String(id),
		payload: { userId: targetUser.id, displayName, source: "bot-command" },
	});
